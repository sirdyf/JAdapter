...
public class JDBTable implements DBInterface {
    ...
    public JDBTable(iConnManager iCM) {
        conMan = iCM;
    }
    public boolean Connect(JTableData jtd, String Url, String UserName, String Password) {
        jData = jtd;
        if (jData==null){
            System.err.println("error: JDBTable().setParam(..) argument JTableData is NULL");
            jData = new JTableData();
            return false;
        }
        conMan.getInstance();
        if (conMan==null){
            System.err.println("error: conMan.getInstance() is NULL");
            //throw new Exception();
            return false;
        }
        this.connection = conMan.CreateExternConnection(Url, UserName, Password);
...
пересмотреть логику работы: ловить исключения на самом низу, сверху логировать. Обеспечивать устойчивость адаптера и
        всех ф-ций.Адаптер сигнализирует об ошибке и возвращает фиктивную таблицу
...
//            //this.CreateModel();//БД изменилась - перечитать данные,чтобы обновить таблицу и модель
        } catch (SQLException e) {
            System.err.println(e);
        }
